...
public class MyPageController {
	...
	@RequestMapping(value = "memInfoUpdate", method = RequestMethod.POST)
	   public String memInfoUpdate(@Validated MemberCommand memberCommand,BindingResult result,
	...
	public String memPwPro(
			@RequestParam(value = "memPw")String memPw,Model model, HttpSession session) {
	...
	public String memPwModify(
			...
			@RequestParam(value = "memberPwCon")String memberPwCon, Model model, HttpSession session) {
		AuthInfoDTO authInfoDTO = (AuthInfoDTO) session.getAttribute("authInfoDTO");
		...
		if(!memberPw.equals(memberPwCon)) {
			model.addAttribute("err_pw_con","비밀번호와 비밀번호 확인 다릅니다.");
			return "thymeleaf/memberShip/memPassCon";
		}
